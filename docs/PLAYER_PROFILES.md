...
Alex and Brad use their existing email/password player accounts for administration. Approved accounts see Administration links on `/profile`, leading to `/admin/analytics` and `/players`. Other players retain their own profile statistics without access to either operator report. The legacy `/admin/login` URL redirects to the normal player account gate; separate admin passwords and old admin cookies are no longer accepted. Signing out of analytics signs out the player account.

Each analytics API request forwards the player's bearer session to Nakama's `pdh_admin_access` RPC. Nakama validates the session and checks its server-only `PDH_ADMIN_USER_IDS` allowlist; the player report uses the same allowlist. There is no localhost bypass, public admin flag, or authority based on a display name/email supplied by the client. Keep exactly Alex and Brad's verified existing account IDs in the production allowlist. Account passwords, balances, IDs and histories are not migrated. Remove obsolete `ADMIN_ANALYTICS_USERS`, `ADMIN_ANALYTICS_USERS_JSON`, `ADMIN_ANALYTICS_PASSCODE` and `ADMIN_ANALYTICS_SESSION_SECRET` settings after deployment; they are no longer read.
